...
s3 = system.read_lammps(fname_solvent)
s3.ff_class = 2
s3.forcefield = 'gaff2'
s3.pair_style = 'lj'
...
s_ = system.replicate([s3], [n_solvent], s_=s, density=None)
# s_.write_lammps(f_lammps_template)

f_solvent_xyz = 'solvent_temp.xyz'
s3.write_xyz(f_solvent_xyz)
...
packmol_inp = packmol_template.replace('OUTPUT_XYZ', f_packmol_xyz).replace('SOLUTE_XYZ', \
                  f_solute_xyz).replace('SOLVENT_XYZ', f_solvent_xyz).replace('N_SOLVENT', \
